...
# Permission constants mapped to minimum role level
...
def _get_teamshifts_role(user, organizer, event, request=None):
    ...
    with scopes_disabled():
        teams = Team.objects.filter(
            organizer=organizer,
            members=user,
        ).filter(Q(all_events=True) | Q(limit_events=event)).exclude(teamshifts_role='')
        best = ''
        for team in teams:
        ...
        def wrapper(request, *args, **kw):
            ...
            if permission:
                if has_teamshifts_permission(
                    request.user, request.organizer, request.event, permission, request=request
                ):
                    return function(request, *args, **kw)
            else:
                if has_any_teamshifts_permission(
